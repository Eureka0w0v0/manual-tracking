...
- **`screen`**：彩色玻璃长方体（v1 后半）——**参数化刚体**：手不钉顶点，只给低噪参数（每手 掌心↔指弧中点插值 = 锚点 → 长轴与盒长；指弧展开量 → 盒高；掌宽比 → 长轴深度分量；掌面前缩 → 绕长轴 roll），长方体在 3D 里造好再弱透视投影，刚性和两点透视是构造出来的；面可见性用 3D 外法线；五指收拢塌成扁带、双手合拢变白色种子点；蓝顶面(反相 LUT+glitch)/绿前面/底面 X-ray/背面红。常数标定见 [`docs/GLASS_BOX_GEOMETRY.md`](docs/GLASS_BOX_GEOMETRY.md)
