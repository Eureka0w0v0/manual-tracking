...
BOX_ROLL_EXPO = 1.5
# 掌宽比 → 长轴深度分量; 一只手往前伸盒子就指向镜头(0 = 长轴锁在像平面)
BOX_AXIS_Z_GAIN = 1.2
...
# 所以超过这个速率的一律是误检。限幅作用在滤波之后, 直接约束"看到的"角速度。
#
# 20 → 10 的依据(tools/e2e_check.py 的 expo×cap 网格, 样片 154 双手帧):
#   cap  切换/s  背≥50%  顶均
#    20   6.43    33.8%  23.5%   ← 旧值, 超过"~5 就是频闪"的线
#    15   5.84    32.5%  27.4%
#    12   5.26    33.8%  27.5%
#    10   4.87    36.4%  32.3%   ← 现值: 进线内, 且背面反而更读得出
#     8   4.68    39.6%  33.2%
#   (那是 07-26 的网格。ebfed26 加了收起/出现过渡并让 e2e 跳过过渡帧后, 同一
#    素材重跑: 20→5.70 / 15→5.49 / 10→4.44 / 8→4.23, 结论不变, 见 docs §3.7 复测)
# 压切换率**不以牺牲可读性为代价**: 限幅挡掉的是饱和区翻符号的瞬移, ψ 少被
# 甩出去, 停留在"看得见背面"区间的时间反而更长。cap=8 数字更好看但
# 8°/帧 @30fps = 240°/s, 已经低于真实快速翻腕(~300°/s), 会削真动作, 不取。
# 10°/帧 = 300°/s 仍在真手速之上; expo 对切换率的影响远小于 cap, 保持 1.5。
BOX_ROLL_MAX_RATE = 10.0
